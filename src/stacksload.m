function [stacks, metadata] = stacksload(stackspath, nbplanes, nbchannels)
    % STACKSLOAD load stacks of images as tensors
    %
    % [stacks, metadata] = stacksload(stackspath, nbplanes, nbchannels)
    %
    % INPUTS
    ...
    %   nbplanes - (optional) default: []
    %       number of z-planes interleaved in stack frames
    %   nbchannels - (optional) default: []
    ...
    %   stack = stacksload('folderpath', 4, 2)
    %
    ...
    function check_posint(s, str)
        posint_attr = {'scalar', 'integer', 'positive'};
        validateattributes(s, {'numeric'}, posint_attr, 'stacksload', str);
    end

    if ~exist('nbplanes', 'var')
        nbplanes = [];
    elseif ~isempty(nbplanes)
        check_posint(nbplanes, 'nbplanes');
    end

    if ~exist('nbchannels', 'var')
        nbchannels = [];
    elseif ~isempty(nbchannels)
        check_posint(nbchannels, 'nbchannels');
    end
